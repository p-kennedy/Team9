    ...
    public static void showUSB() {
        ...
        for (int i = 1; i <= usb.busCount(); i++) {
            ...
            for (int j = 1; j <= usb.deviceCount(i); j++) {
                int vendorId = usb.hexVendorID(i, j); // Get vendor ID
                String vendorName = VendorMapper.getVendorName(vendorId); // Get vendor name
                int productId = usb.productID(i, j); // Get product ID

                // Output the vendor ID and name
                System.out.printf("Bus %d device %d has vendor ID %s (%s) and product ID 0x%04X%n",
                                  i, j, String.format("0x%04X", vendorId), vendorName, productId);
            }
        }
    }
